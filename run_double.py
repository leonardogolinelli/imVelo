...
for K in [31,11]:
    for i in range(2):
        ...
        save_adata(adata, dataset_name, K, knn_rep, save_first_regime=False)"""
        os.rename("outputs", f"outputs_{dataset_name}_K{K}_256_isomap_full_{i}")
...
knn_rep = ["ve", "pca", "pca", "isomap", "isomap"]
best_key = [None, "pca_unique", None, "isomap_unique", None]

for knn_rep, best_key in zip(knn_rep, best_key):
    for K in [31,11]:
        for i in range(5):
            # Run preprocessing
            adata = setup_adata(dataset_name=dataset_name,
