import scanpy as sc
from velocity_coherence_cosine import *
import pickle
...
dic_path = "coherences_cosine.pkl"
abs_dic_path = "absolute_coherences_cosine.pkl"

# Load the coherence dictionaries from Pickle files if they exist
try:
    with open(dic_path, "rb") as pickle_file:
        coherences_cosine = pickle.load(pickle_file)
...
try:
    with open(abs_dic_path, "rb") as abs_pickle_file:
        absolute_coherences_cosine = pickle.load(abs_pickle_file)
except FileNotFoundError:
    # Initialize an empty dictionary if Pickle file does not exist
    absolute_coherences_cosine = {}
...
for dataset, cell_type_key in zip(datasets, cell_type_keys):
    ...
    if dataset not in coherences_cosine:
        coherences_cosine[dataset] = {}
    if dataset not in absolute_coherences_cosine:
        absolute_coherences_cosine[dataset] = {}
    
    for model in models:
        # Check if the model has already been processed for the dataset in both dictionaries
        if model not in coherences_cosine[dataset] or model not in absolute_coherences_cosine[dataset]:
            ...
            coherences_cosine[dataset][model] = adata.obs["velocity_coherence"].tolist()  # Convert to list for Pickle compatibility
            
            # Store the absolute velocity coherence in the dictionary
            absolute_coherences_cosine[dataset][model] = adata.obs["absolute_velocity_coherence"].tolist()  # Convert to list
            
            # Save both dictionaries to Pickle files
            with open(dic_path, "wb") as pickle_file:
                pickle.dump(coherences_cosine, pickle_file)
            with open(abs_dic_path, "wb") as abs_pickle_file:
                pickle.dump(absolute_coherences_cosine, abs_pickle_file)
