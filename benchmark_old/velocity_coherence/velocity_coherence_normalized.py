...
def compute_velocity_coherence(adata, dataset_name, cell_type_key, plot=True):
    """
    Compute velocity coherence and plot violin plot per cell type, with normalized velocity vectors.
    ...
    # Normalize the velocity vectors for each cell
    ...
    # Compute product_score (with directionality) using normalized velocity
    ...
    # Optional: plot violin plot of velocity coherence per cell type
    if plot:
        sc.pl.violin(adata, keys=["velocity_coherence", "absolute_velocity_coherence"], groupby=cell_type_key)
