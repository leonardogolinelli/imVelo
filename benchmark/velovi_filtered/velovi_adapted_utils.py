...
def save_files(adata, vae, dataset):
    try:
        os.remove(f"{dataset}/velovi_filtered_{dataset}/model.pt")
    except:
        print()
    try:
        os.rmdir(f"{dataset}/velovi_filtered_{dataset}")
    except:
        print()

    try:
        os.remove(f"{dataset}/velovi_filtered_{dataset}.h5ad")
    except:
        print()

    adata.write_h5ad(os.path.expanduser(f"{dataset}/velovi_filtered_{dataset}.h5ad"))
    VELOVI.save(vae, os.path.expanduser(f"{dataset}/velovi_filtered_{dataset}"))
