...
def compute_velocity_coherence(adata, dataset_name, cell_type_key):
    ...
    adata.layers["velocity_normalized"] = adata.layers["velocity"] / (velocity_norms + 1e-9)  # Add a small value to avoid division by zero

    prod_normalized = adata.layers["Ms_delta"] * adata.layers["velocity_normalized"]
    adata.layers["product_score"] = prod_normalized

    adata.obs["coherence_hadamard"] = prod.mean(axis=1)  # Convert matrix to array if necessary
    adata.obs["coherence_hadamard_normalized"] = prod_normalized.mean(axis=1)  # Convert matrix to array if necessary
