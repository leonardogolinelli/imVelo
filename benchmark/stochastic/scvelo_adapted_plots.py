...
def plot_phase_plane(adata, gene_name, u_scale=.01, s_scale=0.01, alpha=0.5, head_width=0.02, head_length=0.03, length_includes_head=False, log=False,
    ...
    try:
        ...
        print(f"gene_name location not found")
        return 0

    if smooth_expr:
        unspliced_expression = adata.layers["Mu"][:, idx].flatten() 
        spliced_expression = adata.layers["Ms"][:, idx].flatten() 
    else:
        unspliced_expression = adata.layers["unspliced"][:, idx].flatten()
        spliced_expression = adata.layers["spliced"][:, idx].flatten()
