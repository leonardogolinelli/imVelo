...
model_names = ["velovi", "velovi_filtered"]

for model_name in model_names:
    print(f"Processing model: {model_name}")
    for dataset in datasets:
        print(f"Processing dataset: {dataset}")
        adata = sc.read_h5ad(f"../{model_name}/{dataset}/{model_name}_{dataset}.h5ad")
        ...
        model_path = f"../{model_name}/{dataset}/{model_name}_{dataset}"
        ...
        np.save(f"../matrices/matrix_folder/test_mse_{model_name}_{dataset}", mse_per_cell_test)
