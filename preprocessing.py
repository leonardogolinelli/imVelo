...
def setup_adata(dataset_name='pancreas', 
    ...
    if preproc_adata:
        ...
        if dataset_name == "forebrain":
            sc.tl.umap(adata)
        print(f"adata shape preproc: {adata.shape}")
        select_terms = adata.varm['I'].sum(0) > if preproc_adata:
    if (unspliced_key != "unspliced") and (spliced_key != "spliced"):
        adata.rename({unspliced_key: 'unspliced'})
        adata.rename({spliced_key: 'spliced'})
    
    # Create output directories if they don't exist
    output_adata_dir = "outputs/adata_preproc"
    output_plot_dir = "outputs/adata_preproc"
    os.makedirs(output_adata_dir, exist_ok=True)
    os.makedirs(output_plot_dir, exist_ok=True)

    # Download dataset
    output_adata_path = f"{output_adata_dir}/adata_{dataset_name}_Ksmooth{smooth_k}.h5ad"
    output_plot_path = f"{output_plot_dir}/umap_{dataset_name}_Ksmooth{smooth_k}.png"
    ...
    adata_path = f"12 #remove the terms that contain less than 12 genes AFTER gene filtering
        adata.uns['terms'] = np.array(adata.uns['terms'])[select_terms].tolist()
        adata.varm['I'] = adata.varm['I'][:, select_terms]
        adata._inplace_subset_var(adata.varm['I'].sum(1) > 0) #remove the genes that were only present in terms that have been filtered out in the select_term part
        rename_duplicate_terms(adata)
